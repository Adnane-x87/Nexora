...
function renderCategories() {
  ...
  grid.querySelectorAll('.category-card').forEach(card => {
    card.addEventListener('click', () => {
      window.location.href = '/shop?category=' + encodeURIComponent(card.dataset.cat);
    });
  });
}
...
function renderProducts() {
  ...
  const products = loadProducts();
  if (!products.length) {
    grid.innerHTML = `<div class="empty-products"><div class="e-icon">📦</div><div class="e-title">No products yet</div><p>Explore our shop for more!</p></div>`;
    return;
  }
